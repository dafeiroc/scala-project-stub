...
object Projects extends Build {
  ...
  lazy val api = Project(
    ...
    .settings(libraryDependencies ++=
      compile(akkaActor, akkaSlf4j, akkaHttp, logback, playJson, scalaTime, cats) ++
        test(scalaTest, scalaCheck, akkaTest, akkaHttpTest))
    .dependsOn(domain)
  ...
  lazy val domain = Project(
    id = "domain",
    base = file("support/domain")
  )
    .settings(moduleName := "domain")
    .settings(libraryDependencies ++=
      compile(scalaTime, cats, mysql, slick, slickCodegen) ++
